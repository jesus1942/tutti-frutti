"""
Gestor de estadísticas del juego Tutti Frutti

Esta clase se encarga de:
- Cargar y guardar estadísticas de jugadores
- Actualizar métricas de jugadores
- Registrar partidas completadas
- Gestionar guardado automático
...
class StatsManager:
    ...
    def __init__(self, stats_file="player_stats.json"):
        """
        Inicializa el gestor de estadísticas
        
        Args:
            stats_file (str): Ruta al archivo de estadísticas
        """
    ...
    def start_auto_save(self, interval=60):
        """
        Iniciar guardado automático de estadísticas
        
        Args:
            interval (int): Intervalo en segundos entre guardados
        """
        def auto_save():
            while self.running:
                time.sleep(interval)
        ...
        print(f"Guardado automático de estadísticas iniciado (cada {interval}s)")
    ...
    def update_player_stats(self, player_name, event_type, data=None):
        ...
        Args:
            ...
            event_type (str): Tipo de evento ('connect', 'game_join', 'round_complete', 'points')
            data (int, dict, optional): Datos adicionales del evento
        """
        with self.lock:
    ...
    def add_game_record(self, game_data):
        ...
        Args:
            game_data (dict): Datos de la partida completada
        """
        with self.lock:
    ...
    def update_active_rooms(self, count):
        ...
        Args:
            count (int): Número de salas activas
        """
        with self.lock:
